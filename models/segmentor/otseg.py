...
class OTSeg(EncoderDecoder):
    ...
    def __init__(self,
                 ...
                 **args):
        super(OTSeg, self).__init__(**args)
        ...
        self.num_classes = len(self.both_class)
        self.N = number_prompts
        if not self.load_text_embedding:
            if hasattr(self.backbone, 'num_tokens') :
                ...
                print("Hand-Crafted Multiple text prompt!!!")
                self.texts = []
                for template in templates:
                    template_embeddings = []
                    for c in class_names:
                        t = tokenize(template.format(c))
                        template_embeddings.append(t)
                    t_s = torch.cat(template_embeddings)
                    self.texts.append(t_s)
                self.texts = torch.stack(self.texts, dim=1)
                
            else:
                print("Learnable Multiple text prompt!!!")
                self.texts = torch.cat([tokenize(c, context_length=self.context_length) for c in class_names]).unsqueeze(1)
                self.texts = self.texts.repeat(1, self.N, 1)
                context_length = self.text_encoder.context_length - self.context_length
                token_embed_dim = 512
                self.contexts = nn.Parameter(torch.randn(self.N, context_length, token_embed_dim))
    ...
    def _decode_head_forward_train(self, feat, img_metas, gt_semantic_seg):
        ...
        if self.self_training:
            loss_decode, loss_score = self.decode_head.forward_train(feat, 
                                                        img_metas,
                                                        gt_semantic_seg,
                                                        self.train_cfg,
                                                        self.self_training,
        ...
        losses.update(add_prefix(loss_score, 'score'))
        return losses
    ...
    def text_embedding(self, texts, img):
        if hasattr(self.backbone, 'num_tokens'):
            ...
            text_embeddings = self.text_encoder(texts.to(img.device), self.contexts)
